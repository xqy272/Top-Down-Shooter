...
namespace Player
{
    public class PlayerAim : MonoBehaviour
    {
        ...
        private void UpdateAimLaser()
        {
            ...
            Vector3 secondPoint = firstPoint.position + laserDirection * laserLength;
        
            if (Physics.Raycast(firstPoint.position, laserDirection, out RaycastHit hit, laserLength))
            {
                secondPoint = hit.point;
                // laserEndWidth = (laserLength - (firstPoint.position - secondPoint).magnitude) % laserLength * laserStartWidth;
